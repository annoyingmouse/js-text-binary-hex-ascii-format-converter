...
export const getOutPutText = data => {
  ...
  if (data['output-type'].includes('binary')) {
    returnString += formatOutput('Binary', textToBinary(getTransitoryValue(data)))
  }
  if (data['output-type'].includes('hex')) {
    returnString += formatOutput('Hexadecimal', textToHex(getTransitoryValue(data)))
  }
  if (data['output-type'].includes('ascii')) {
    returnString += formatOutput('Ascii', textToAscii(getTransitoryValue(data)))
  }
